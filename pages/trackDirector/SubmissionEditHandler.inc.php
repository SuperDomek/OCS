...
class SubmissionEditHandler extends TrackDirectorHandler {
			...
			foreach ($submission->getReviewAssignments($stage) as $reviewAssignment) {
				...
				*/
				...
				// EDIT Added else branch
					...
					if($sessionType == 7) { // SessionType Research Article
						TrackDirectorAction::addReviewForm($submission, $reviewAssignment->getId(), 8);
						// see table review_form_settings
						header("Refresh:0"); // reload page
						exit();	// stop executing code so page reloads instanteously
					}
					elseif($sessionType == 8){ // SessionType Review Study
						TrackDirectorAction::addReviewForm($submission, $reviewAssignment->getId(), 9);
						header("Refresh:0");
						exit();
					}
	...
	function recordDecision($args) {
		...
		if (
			...
			//	Edit Simply set up the decision without switching
			//	This switch only makes it more difficult to change the decision options,
			//	because it is set up in \classes\submission\trackDirector\TrackDirectorSubmission.inc.php.
			/*switch ($decision) {
